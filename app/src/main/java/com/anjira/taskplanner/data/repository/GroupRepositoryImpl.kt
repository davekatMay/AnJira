...
class GroupRepositoryImpl(
    ...
    override suspend fun getUserGroups(): List<Group> = apiOrCache(
        api = { withContext(Dispatchers.IO) { apiService.getUserGroups().execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); (r.body() ?: emptyList()).map { it.toGroup() } } } },
    ...
    override suspend fun getGroupDetail(groupId: Int): Group = apiOrCache(
        api = { withContext(Dispatchers.IO) { apiService.getGroupDetail(groupId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); checkBody(r).toGroup() } } },
        cache = { throw Exception("No cache") }
    )
    ...
    override suspend fun removeGroupMember(groupId: Int, userId: Int) =
        withContext(Dispatchers.IO) { apiService.removeGroupMember(groupId, userId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error") } }

    override suspend fun updateMemberRole(groupId: Int, memberId: Int, role: String) =
        withContext(Dispatchers.IO) { apiService.updateMemberRole(groupId, memberId, UpdateRoleRequest(role)).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error") } }

    override suspend fun getGroupTasks(groupId: Int, filter: String?, status: String?): List<Task> = apiOrCache(
        api = { withContext(Dispatchers.IO) { apiService.getGroupTasks(groupId, filter, status).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); (r.body() ?: emptyList()).map { it.toTask() } } } },
    ...
    override suspend fun getGroupMeetings(groupId: Int): List<Meeting> = apiOrCache(
        api = { withContext(Dispatchers.IO) { apiService.getGroupMeetings(groupId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); (r.body() ?: emptyList()).map { it.toMeeting() } } } },
    ...
    override suspend fun addMeetingParticipant(meetingId: Int, userId: Int) =
        withContext(Dispatchers.IO) { apiService.addMeetingParticipant(meetingId, AddParticipantRequest(userId)).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error") } }

    override suspend fun removeMeetingParticipant(meetingId: Int, userId: Int) =
        withContext(Dispatchers.IO) { apiService.removeMeetingParticipant(meetingId, userId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error") } }

    override suspend fun getMeetingParticipants(meetingId: Int): List<MeetingParticipant> =
        withContext(Dispatchers.IO) { apiService.getMeetingParticipants(meetingId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); (r.body() ?: emptyList()).map { MeetingParticipant(it.userId, it.username, it.email, it.status) } } }

    override suspend fun updateRsvp(meetingId: Int, participantId: Int, status: String) =
        withContext(Dispatchers.IO) { apiService.updateRsvp(meetingId, participantId, RsvpRequest(status)).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error") } }

    override suspend fun getAnnouncements(groupId: Int): List<Announcement> = apiOrCache(
        api = { withContext(Dispatchers.IO) { apiService.getAnnouncements(groupId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); (r.body() ?: emptyList()).map { it.toAnnouncement() } } } },
    ...
    override suspend fun togglePinAnnouncement(announcementId: Int) =
        withContext(Dispatchers.IO) { apiService.togglePinAnnouncement(announcementId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error") } }

    override suspend fun getPlaylists(groupId: Int): List<Playlist> = apiOrCache(
        api = { withContext(Dispatchers.IO) { apiService.getPlaylists(groupId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); (r.body() ?: emptyList()).map { it.toPlaylist() } } } },
    ...
    override suspend fun deletePlaylist(playlistId: Int) =
        withContext(Dispatchers.IO) { apiService.deletePlaylist(playlistId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error") } }

    override suspend fun getTracks(playlistId: Int): List<PlaylistTrack> = apiOrCache(
        api = { withContext(Dispatchers.IO) { apiService.getTracks(playlistId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); (r.body() ?: emptyList()).map { it.toPlaylistTrack() } } } },
    ...
    override suspend fun reorderTracks(playlistId: Int, trackIds: List<Int>) =
        withContext(Dispatchers.IO) { apiService.reorderTracks(playlistId, ReorderTracksRequest(trackIds)).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error") } }

    override suspend fun searchItunes(term: String): String =
        withContext(Dispatchers.IO) { apiService.searchItunes(term).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); r.body() ?: "[]" } }

    override suspend fun getItunesTrack(trackId: String): String =
        withContext(Dispatchers.IO) { apiService.getItunesTrack(trackId).execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); r.body() ?: "{}" } }

    override suspend fun getUserStats(): UserStats =
        withContext(Dispatchers.IO) { apiService.getUserStats().execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); checkBody(r).toUserStats() } }

    override suspend fun getNotifications(): List<AppNotification> =
        withContext(Dispatchers.IO) { apiService.getNotifications().execute().let { r -> if (!r.isSuccessful) throw Exception(r.errorBody()?.string() ?: "Unknown error"); (r.body() ?: emptyList()).map { AppNotification(it.id, it.type, it.referenceId, it.message, it.isRead, it.createdAt) } } }
